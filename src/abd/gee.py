...
def extract_latitude_longitude_pixel(
    ...
    coordinates = image.addBands(image.pixelLonLat()).select(
...
def apply_masks(image, params: dict):
    ...
    slope = (
        ...
        .rename("slope")
        .cast({"slope": "double"})
    )  # Ogashawara, Li, and Moreno-Madrian (2017)
    label = image.expression(
        "((cloud == 1) ? -1 : (slope > -0.05)+(fai > -0.004)+(ndvi > -0.15)+(sabi > -0.10))",
        {
            "slope": slope.select("slope"),
            "fai": fai.select("fai"),
            "ndvi": ndvi.select("ndvi"),
            "sabi": sabi.select("sabi"),
            "cloud": cloud.select("cloud"),
        },
    ).rename("label")
    occurrence = label.expression(
        "(label >= 3) ? 1 : 0", {"label": label.select("label")}
    ).rename("occurrence")
