...
class Abd:
    ...
    def __init__(
        ...
        collection = gee.get_sensor_collections(
            geometry=self.geometry,
            sensor=self.sensor,
            dates=[
                dt.strftime(self.date - td(days=16), "%Y-%m-%d"),
                dt.strftime(self.date + td(days=16), "%Y-%m-%d"),
            ],
        )
        ...
        for d in dates_list:
            if dt.strftime(self.date, "%Y-%m-%d") == dt.strftime(d, "%Y-%m-%d"):
                ...
                self.date = (
                    d
                    if abs((d - self.date).days) <= abs((self.date - last).days)
                    else last
                )
                break
            last = d
        ...
        if self.dates_timeseries[1] > dt.now():
            self.dates_timeseries[0] -= td(
                days=(self.dates_timeseries[1] - dt.now()).days
            )
        ...
        if self.dates_timeseries[0] < self.sensor_params["start"]:
            self.dates_timeseries[1] += td(
                days=(self.sensor_params["start"] - self.dates_timeseries[0]).days
            )
        ...
        self.sample_lon_lat = [
            [float(coordinates_min["latitude"]), float(coordinates_min["longitude"])],
            [float(coordinates_max["latitude"]), float(coordinates_max["longitude"])],
        ]
        ...
        self.image_clip = self.clip_image(
            self.extract_image_from_collection(self.date),
            geometry=self.geometry,
            scale=self.scale,
        )
        ...
        for i, geometry in enumerate(self.splitted_geometry):
            ...
            if not self.image_clip is None:
                ...
                # count image pixels
                counters = (
                    self.image_clip.select(
                        [self.sensor_params["red"], "water", "water_nocloud"]
                    )
                    .reduceRegion(
                        reducer=ee.Reducer.count(), geometry=geometry, scale=self.scale
                    )
                    .getInfo()
                )
    ...
    def apply_kmeans(self, df: pd.DataFrame):
        ...
        diff_attributes = self.attributes_extra + [
            a + "_diff" for a in self.attributes if a != "cloud"
        ]

        # get gridsearch data
        X = self.scaler.fit_transform(
            df[diff_attributes].values.reshape((-1, len(diff_attributes)))
        )
    ...
    def extract_image_pixels(self, image: ee.Image, date):
            ...
            try:
                ...
                for i, geometry in enumerate(self.splitted_geometry):
                    ...
                    clip = self.clip_image(
                        self.extract_image_from_collection(date=date),
                        geometry=geometry,
                        scale=self.scale,
                    )
                    if not clip is None:
                        ...
                        if (
                            ...
                            geometry_lons_lats_attributes = (
                                gee.extract_latitude_longitude_pixel(
                                    image=clip,
                                    geometry=geometry,
                                    bands=[a + "_water" for a in self.attributes_full],
                                    scale=self.scale,
                                )
                            )
        ...
        try:
            ...
            df_timeseries = (
                pd.DataFrame(data=lons_lats_attributes, columns=self.df_columns)
                .infer_objects()
                .sort_values(["lat", "lon"])
    ...
    def fix_timeseries(self, df: pd.DataFrame):
        ...
        ).dt.dayofyear.astype(dtype=np.int64, errors="ignore")
        ...
        if self.image_is_good:
            ...
            for attribute in attributes:
                df_timeseries = df_timeseries[
                    (df_timeseries[attribute] != abs(self.dummy))
                ]
            ...
            df_timeseries.drop_duplicates(
                subset=["pixel", "date", "lat", "lon"] + attributes,
                keep="last",
                inplace=True,
            )

            # create median timeseries columns
            df_timeseries_median = (
                df_timeseries.copy(deep=True)
                .groupby(["pixel"])
                .median()
                .reset_index()
                .drop(["index", "doy", "cloud"], axis=1)
                .sort_values(by=["pixel"])
            )
            for attribute in attributes_median:
                df_timeseries[attribute] = np.nan
            first_date = dt.strptime(
                df_timeseries["date"].unique()[0].astype(str).split("T")[0], "%Y-%m-%d"
            )
            pixels = (
                df_timeseries[
                    (df_timeseries["pixel"].isin(df_timeseries_median["pixel"].values))
                    & (df_timeseries["date"] == first_date)
                ]
                .groupby(["pixel"])
                .median()
                .reset_index()["pixel"]
            ...
            df_timeseries.loc[
                (df_timeseries["pixel"].isin(pixels))
                & (df_timeseries["date"] == first_date),
                attributes_median,
            ] = df_timeseries_median[df_timeseries_median["pixel"].isin(pixels)][
                attributes
            ].values
            df_timeseries = (
                df_timeseries.sort_values(by=["pixel", "date"])
                .fillna(method="ffill")
                .sort_values(by=["date", "pixel"])
            )

            # save modified dataframe to its original variable
            self.df_timeseries = df_timeseries[
                self.df_columns + attributes_median
            ].dropna()
        ...
        if self.image_is_good:
            ...
            if self.remove_outliers:
                ...
                self.df_train = self.df_train[
                    (
                        np.abs(
                            scipy.stats.zscore(self.df_train[["index"] + attributes])
                        )
            ...
            self.df_gridsearch.loc[
                self.df_gridsearch["index"].isin(self.df_train["index"].values), "label"
            ] = 0
    ...
    def train(self):
        ...
        if self.image_is_good:
            ...
            # split data
            X_gridsearch = self.scaler.fit_transform(
                self.df_gridsearch[attributes].values.reshape((-1, len(attributes)))
            )
            y_gridsearch = self.df_gridsearch["label"].values.reshape(1, -1)[0]
            X_train = self.scaler.transform(
                self.df_train[attributes].values.reshape((-1, len(attributes)))
            )
            ...
            if self.model is None or self.model == "ocsvm":
                ...
                # gridsearch data
                X_gridsearch_, _, y_gridsearch_, _ = model_selection.train_test_split(
                    X_gridsearch,
                    self.encode_ocsvm_labels(y_gridsearch),
                    train_size=0.01,
                    shuffle=True,
                    random_state=self.random_state,
                )
                ...
                random_grid = [
                    {
                        "kernel": ["rbf"],
                        "gamma": scipy.stats.expon(scale=0.100),
                        "nu": scipy.stats.expon(scale=0.100),
                        "degree": [3],
                        "coef0": [0.0],
                ...
                # initialize searching process for OneClassSVM's best
                # parameters selection
                start_time = time.time()
                rs = model_selection.RandomizedSearchCV(
                    estimator=svm.OneClassSVM(verbose=False),
                ...
                X_train_, X_test, y_train_, y_test = model_selection.train_test_split(
                    X_train,
                    self.encode_ocsvm_labels(y_train),
                    train_size=0.01,
                    shuffle=True,
                    random_state=self.random_state,
                )
                ...
                str_model = (
                    "OneClassSVM (kernel="
                    + str(rs.best_params_["kernel"])
                    + ",gamma="
                    + str(rs.best_params_["gamma"])
                    + ",nu="
                    + str(rs.best_params_["nu"])
                    + ",shrinking="
                    + str(rs.best_params_["shrinking"])
                    + ")"
                )
            ...
            if self.model is None or self.model == "rf":
                ...
                random_grid = {
                    "n_estimators": np.linspace(
                        1, 250, num=51, dtype=int, endpoint=True
                    ),
                ...
                # initialize searching process for Random Forest's
                # best parameters selection
                start_time = time.time()
                rs = model_selection.RandomizedSearchCV(
                    estimator=ensemble.RandomForestClassifier(
                        n_jobs=self.n_cores, random_state=self.random_state
                    ),
                    param_distributions=random_grid,
                    scoring={
                        "accuracy": metrics.make_scorer(metrics.accuracy_score),
            ...
            if self.model is None or self.model == "if":
                ...
                X_gridsearch_, _, y_gridsearch_, _ = model_selection.train_test_split(
                    X_gridsearch,
                    self.encode_if_labels(y_gridsearch),
                    train_size=0.01,
                    shuffle=True,
                    random_state=self.random_state,
                )
                ...
                # train data
                X_train_, X_test, _, y_test = model_selection.train_test_split(
                    X_gridsearch,
                    y_gridsearch,
                    train_size=0.70,
                    shuffle=True,
                ...
                str_model = (
                    ...
                    + str(rs.best_params_["n_estimators"])
                    + ",max_samples="
                    + str(rs.best_params_["max_samples"])
                    + ",contamination="
                    + str(rs.best_params_["contamination"])
                    + ",max_features="
                    + str(rs.best_params_["max_features"])
                    + ",bootstrap="
                    + str(rs.best_params_["bootstrap"])
    ...
    def detect(self):
        ...
        if isinstance(self.df_image, pd.DataFrame) and not self.df_image.empty:
            ...
            X = self.scaler.transform(
                self.df_image[attributes].values.reshape((-1, len(attributes)))
            )
            ...
            for i, attribute in enumerate(attributes):
                ...
                for index, row in self.df_image.iterrows():
                    if attribute in self.attributes_inverse:
                        label = (
                            self.anomaly
                            if row[attribute] < self.indices_thresholds[attribute]
                            else 0
                        )
                    else:
                        label = (
    ...
    def save_detection_plot(self, path: str):
        ...
        if isinstance(self.df_image, pd.DataFrame) and not self.df_image.empty:
            ...
            # axis ticks
            xticks = np.linspace(
                self.df_image["lat"].min(), self.df_image["lat"].max(), num=4
            )
            yticks = np.linspace(
                self.df_image["lon"].min(), self.df_image["lon"].max(), num=4
            )
            ...
            markersize = (72.0 / fig.dpi) * multiplier
            ...
            # handles legend
            handles = [
                Patch(color="cyan", label="Regular"),
                Patch(color="red", label="Anomaly"),
                Patch(color="gray", label="Cloud"),
                Patch(color="white", label="Land/Absence"),
            ]
            ...
            for i, model in enumerate(self.classifiers):
                ...
                count += 1
                try:
                    image_rgb_clip = self.image_clip.select(
                        self.sensor_params["red"],
                        self.sensor_params["green"],
                        self.sensor_params["blue"],
                    ).getThumbUrl({"min": 0, "max": 3000, "dimensions": 500})
                    imageIO = PIL.Image.open(
                        BytesIO(requests.get(image_rgb_clip, timeout=600).content)
                    )
                except Exception:
                ...
                c.set_title(
                    "Natural color [Red, Green, Blue]", fontdict={"fontsize": 7}
                )
                c.set_xticks(xticks)
                c.set_yticks(yticks)
                ...
                count += 1
                try:
                    image_false_clip = self.image_clip.select(
                        self.sensor_params["nir"],
                        self.sensor_params["red"],
                        self.sensor_params["green"],
                    ).getThumbUrl({"min": 0, "max": 3000, "dimensions": 500})
                    imageIO = PIL.Image.open(
                        BytesIO(requests.get(image_false_clip, timeout=600).content)
                    )
                except Exception:
                    imageIO = imageIO_blank
                ...
                for attribute in attributes:
                    ...
                    s.set_clim(
                        [
                            self.indices_thresholds[attribute],
                            self.df_image[attribute].max(),
                        ]
                    )
                    cb = plt.colorbar(
                        s,
                        cax=make_axes_locatable(c).append_axes(
                    ...
                    c.scatter(
                        self.df_image[self.df_image["cloud"] == 1]["lat"],
                        self.df_image[self.df_image["cloud"] == 1]["lon"],
                        marker="s",
                        s=markersize,
                        c="gray",
                    )
                ...
                c = fig.add_subplot(rows, columns, count)
                c.set_title(
                    "Anomalous Behaviour Detection (ABD)", fontdict={"fontsize": 7}
                )
                c.set_xticks(xticks)
                c.set_yticks(yticks)
                c.yaxis.tick_right()
                c.scatter(
                    self.df_image[self.df_image["label_" + str(i)] != self.anomaly][
                        "lat"
                    ],
                    self.df_image[self.df_image["label_" + str(i)] != self.anomaly][
                        "lon"
                ...
                c.imshow(
                    imageIO_blank, extent=[xticks[0], xticks[-1], yticks[0], yticks[-1]]
                )
                c.margins(x=0, y=0)
                c.xaxis.set_major_formatter(FormatStrFormatter("%.2f"))
                c.yaxis.set_major_formatter(FormatStrFormatter("%.2f"))
    ...
    def validate_using_roi(
        self, path: str, rois: list, labels: list = [], df_pred: pd.DataFrame = None
    ):
        ...
        if isinstance(self.df_image, pd.DataFrame) and not self.df_image.empty:
                ...
                try:
                    ...
                    for geometry in self.splitted_geometry:
                        clip = self.clip_image(
                            image=ee.Image(2)
                            .toByte()
                            .paint(ee.FeatureCollection(path + "/" + roi), labels[i]),
                            geometry=geometry,
                            scale=self.scale,
                        )
                        geometry_lons_lats_attributes = (
            ...
            df_pred_raw = (
                df_pred.copy(deep=True)
                if df_pred is not None
                else self.df_image.copy(deep=True)
            )
            ...
            for i, model in enumerate(self.classifiers):
                ...
                # prepare model data
                df_pred = df_pred_raw[["lat", "lon", "label_" + str(i)]].rename(
                    columns={"label_" + str(i): "label"}
                )

                # merge true and pred dataframes, finding matched pair
                ...
                # get predictions on test set
                y_true, y_pred = merged["label_true"].values.astype(int), merged[
                    "label_pred"
                ].values.astype(int)
                measures = misc.concordance_measures(
                    metrics.confusion_matrix(y_true, y_pred), y_true, y_pred
            ...
            for indice in self.indices_thresholds:
                ...
                if indice in self.attributes:
                    ...
                    # prepare model data
                    df_pred = df_pred_raw[["lat", "lon", indice]].rename(
                        columns={indice: "label"}
                    )
                    df_pred.loc[
                        df_pred["label"] > self.indices_thresholds[indice], "label"
                    ] = (self.anomaly if indice not in self.attributes_inverse else 0)
                    df_pred.loc[
                        df_pred["label"] <= self.indices_thresholds[indice], "label"
                    ] = (0 if indice not in self.attributes_inverse else self.anomaly)
            ...
            self.df_results = pd.DataFrame(
                dict_results, columns=self.df_columns_results
            )
    ...
    def save_occurrences_plot(self, df: pd.DataFrame, path: str):
        ...
        # axis ticks
        xticks = np.linspace(
            self.sample_lon_lat[0][1], self.sample_lon_lat[1][1], num=4
        )
        yticks = np.linspace(
            self.sample_lon_lat[0][0], self.sample_lon_lat[1][0], num=4
        )

        # colorbar tixks
        colorbar_ticks_max = 100
        colorbar_ticks = np.linspace(
            0, colorbar_ticks_max if colorbar_ticks_max > 1 else 2, num=5, dtype=int
        )
        ...
        fig.suptitle(
            "Algal Bloom Yearly Occurrences  (" + str_date + ")", fontsize=14, y=1.04
        )
        fig.autofmt_xdate()
        ...
        cbar = fig.colorbar(
            images[-1],
            cax=fig.add_axes([0.6, -0.05, 0.39, 0.05]),
            ...
            features.append(
                geojson.Feature(
                    ...
                    properties={
                        ...
                        "not_occurrence": int(row["not_occurrence"]),
                        "pct_occurrence": int(row["pct_occurrence"]),
                        "pct_cloud": int(row["pct_cloud"]),
    ...
    def save_image_png(
        ...
        date: dt,
        path: str,
        options: dict = {"min": 0, "max": 3000, "dimensions": 500},
    ):
        ...
        if self.sensor_params["sensor"] == "landsat578":
            sensor = (
                ...
                .get(self.sensor_params["property_id"])
                .getInfo()
            )
            if "LT05" in sensor:
                bands = [
    ...
    def save_collection_tiff(self, folder: str, folderName: str):
        ...
        for date in self.dates_timeseries_interval:
            image = self.clip_image(
                self.extract_image_from_collection(date=date), geometry=self.geometry
            )
            self.save_image_tiff(
                image=image,
                date=date,
                path=folder + "/" + date.strftime("%Y-%m-%d") + ".zip",
    ...
    def save_collection_png(self, folder: str):
        ...
        for date in self.dates_timeseries_interval:
            ...
            for i, geometry in enumerate(self.splitted_geometry):
                self.save_image_png(
                    image=self.clip_image(
                        self.extract_image_from_collection(date=date), geometry=geometry
                    ),
                    date=date,
                    path=path_image
                    + "/"
