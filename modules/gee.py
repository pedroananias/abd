...
def apply_masks(image, params: dict):
  ...
  slope           = image.expression('((red - nir) / (c_red - c_nir)) * 1000',{'nir':image.select(params['nir']).multiply(0.0001),'red':image.select(params['red']).multiply(0.0001),'c_nir':params['c_nir'],'c_red':params['c_red']}).rename('slope').cast({"slope": "double"}) # Ogashawara, Li, and Moreno-Madrian (2017)
  label           = image.expression('((cloud == 1) ? -1 : (slope > -0.05)+(fai > -0.004)+(ndvi > -0.15)+(sabi > -0.10))', {'slope': slope.select('slope'), 'fai': fai.select('fai'), 'ndvi': ndvi.select('ndvi'), 'sabi': sabi.select('sabi'), 'cloud': cloud.select('cloud')}).rename('label')
  occurrence      = label.expression('(label >= 3) ? 1 : 0', {'label': label.select('label')}).rename('occurrence')

  # Create the bands to the image and return it
  return image.addBands([water, water_nocloud, cloud, nocloud, ndwi, ndvi, sabi, fai, slope, label, occurrence, red, green, blue, nir, swir])
...
def get_image_min_max(image: ee.Image, geometry: ee.Geometry, scale: int = None, tile_scale: int = 16):
  ...
  return image.reduceRegion(reducer=ee.Reducer.min(), geometry=geometry, scale=scale, bestEffort=False, tileScale=tile_scale).getInfo(), image.reduceRegion(reducer=ee.Reducer.max(), geometry=geometry, scale=scale, bestEffort=False, tileScale=tile_scale).getInfo()
...
def get_image_counters(image: ee.Image, geometry: ee.Geometry, scale: int = None, tile_scale: int = 16):
  if scale is None:
    scale = image.projection().nominalScale()
  return image.reduceRegion(reducer=ee.Reducer.count(), geometry=geometry, scale=scale, bestEffort=False, tileScale=tile_scale).getInfo()
